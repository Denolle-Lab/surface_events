...
for n in range(len(evt_id)):    
    ...
    try:
        ...
        if net != 'CN' and evt_id[n]!=evt_id[n-1]:
            ...
            evt_data.to_csv('~/surface_events/Analysis_Data/Event_Data_10_24.csv', index=False)
            sta_freq.to_csv('~/surface_events/Analysis_Data/Station_frequency_data_10_24.csv', index=False)
    except:
        reject_evts = reject_evts.append({'event_ID':[event_ID]}, ignore_index = True)
        reject_evts.to_csv('~/surface_events/Analysis_Data/Rejects_10_24.csv', index=False)
        continue
